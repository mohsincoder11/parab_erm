                    ...
                    <div class="modal fade add-edit_modal" tabindex="-1" role="dialog"
                        ...
                        <div class="modal-dialog modal-lg" role="document">
                            <div class="modal-content">
                                ...
                                <form method="post" id="employee_pf_form" enctype="multipart/form-data">

                                    @csrf

                                    <input type="hidden" name="id" id="employee_pf_id">
                                    <input type="hidden" name="old_file1" id="old_file1">
                                    <input type="hidden" name="old_file2" id="old_file2">
                                    ...
                                    <div class="row" style="padding: 10px;">
                                        ...
                                        <div class="col-md-6 form-group">
                                            ...
                                            <input type="file" class="form-control" name="pf_file" id="pf_file">
                                        ...
                                        <div class="col-md-6 form-group">
                                            ...
                                            <input type="file" class="form-control" name="chalan" id="chalan">
                                            <span id="store_logo2"></span>

                                        </div>

                                        <div class="form-group" align="center" style="margin-top: 15%; ">
                                            <button formaction="{{ route('statutory_masters.store-employee-pf') }}" type="submit"
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.edit', function() {
                let id = $(this).attr('id');
                $.ajax({
                    url: "{{ route('statutory_masters.edit-employee-pf') }}",
                    method: "GET",
                    dataType: 'json',
                    data: {
                        id: id,
                    ...
                    success: function(result) {
                        $("#pf_file").addClass('ignore');
                        $("#chalan").addClass('ignore');
                        $("#employee_pf_id").val(result.id);
                        $("#chartA").val(result.chartA);

                        $("#store_logo1").text(result.chalan);
                        $("#old_file1").val(result.chalan);

                        $("#store_logo2").text(result.pf_file);
                        $("#old_file2").val(result.pf_file);

                          $("#myLargeModalLabel").text("Edit Pf Detail")
                        $("#add-edit-btn").text("Update");
                        let formaction = '{{ route('statutory_masters.update-employee-pf') }}';
            ...
            $('.add-edit_modal').on('hidden.bs.modal', function() {
                $("#myLargeModalLabel").text("Add Pf Detail");
                $("#store_logo").text('');

                $("#add-edit-btn").text("Add");
                $('#employee_pf_form').trigger("reset");
                let formaction = '{{ route('statutory_masters.store-employee-pf') }}';
                $("#add-edit-btn").attr("formaction", formaction);
            ...
            $(document).on('click', '.delete', function() {
                ...
                swal({
                    ...
                    if (result.value) {
                        $.ajax({
                            url: "{{ route('statutory_masters.delete-employee-pf') }}",
                            method: "GET",
                            dataType: 'json',
                            data: {
                                id: id,
            ...
            var table = $('.datatable_server').DataTable({
                ...
                "ajax": {
                    url: "{{ route('statutory_masters.get-employee-pf') }}",
                    type: "get",
                    data: function(d) {}
                },
                columns: [{
