                        ...
                        <div class="modal-dialog modal-lg" role="document">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="myLargeModalLabel">Add New ESIC</h5>
                                ...
                                <form method="post" id="employee_esic_form" enctype="multipart/form-data">

                                    @csrf

                                    <input type="hidden" name="id" id="employee_esic_id">
                                    ...
                                    <div class="row" style="padding: 10px;">
                                        ...
                                        <div class="col-md-6 form-group">
                                            <label>Modify ESIC Master</label>
                                            <input type="file" class="form-control" name="esic_file" id="esic_file">
                                        ...
                                        <div class="form-group" align="center" style="margin-top: 5%; padding-left:45%;">
                                            <button formaction="{{ route('statutory_masters.store-employee-esic') }}" type="submit"
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.edit', function() {
                let id = $(this).attr('id');
                $.ajax({
                    url: "{{ route('statutory_masters.edit-employee-esic') }}",
                    method: "GET",
                    dataType: 'json',
                    data: {
                        id: id,
                    ...
                    success: function(result) {
                        $("#esic_file").addClass('ignore');
                        $("#employee_esic_id").val(result.id);
                        $("#chartA").val(result.chartA);

                        $("#store_logo1").text(result.esic_file);
                        $("#old_file1").val(result.esic_file);


                          $("#myLargeModalLabel").text("Edit ESIC Detail")
                        $("#add-edit-btn").text("Update");
                        let formaction = '{{ route('statutory_masters.update-employee-esic') }}';
            ...
            $('.add-edit_modal').on('hidden.bs.modal', function() {
                $("#myLargeModalLabel").text("Add ESIC Detail");
                $("#store_logo").text('');

                $("#add-edit-btn").text("Add");
                $('#employee_esic_form').trigger("reset");
                let formaction = '{{ route('statutory_masters.store-employee-esic') }}';
                $("#add-edit-btn").attr("formaction", formaction);
            ...
            $(document).on('click', '.delete', function() {
                ...
                swal({
                    ...
                    if (result.value) {
                        $.ajax({
                            url: "{{ route('statutory_masters.delete-employee-esic') }}",
                            method: "GET",
                            dataType: 'json',
                            data: {
                                id: id,
            ...
            var table = $('.datatable_server').DataTable({
                ...
                "ajax": {
                    url: "{{ route('statutory_masters.get-employee-esic') }}",
                    type: "get",
                    data: function(d) {}
                },
                columns: [{
